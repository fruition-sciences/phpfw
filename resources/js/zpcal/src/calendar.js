  ...
  if (oEl.getBoundingClientRect) {
    ...
    if (sPos=='fixed') {
      ...
      while(oP) {
        ...
        iTop+=parseInt(oP.offsetTop,10)||0;oP=oP.offsetParent;
      }
      oP=oEl;
      while(oP.parentNode) {
        oP=oP.parentNode;
...
Zapatec.Utils.getElementOffsetScrollable=function(oEl){var oPos=Zapatec.Utils.getElementOffset(oEl);if(oEl.scrollLeft){oPos.left-=oEl.scrollLeft;oPos.x=oPos.left;}
...
el.calendar.showHint(Zapatec.Calendar.i18n("SEL_DATE"));return Zapatec.Utils.stopEvent(ev);};
    ...
    switch (el.navtype) { 
      ...
      case-1:
        if (mon>0) {
          ...
          date.setFullYear(year);setMonth(11);
        }
        break;
      case 1:
        if(mon<11) {
      ...
      case 50:
        ...
        else if (!((cal.timeInterval==null) || ((cal.timeInterval<60) && (el.className.indexOf("hour",0) != -1)))) {
          break;
        }
        var range = el._range;
      ...
      case 201: case 202:
        ...
        return;
      case 0:
        if (cal.getDateStatus&&((cal.getDateStatus(date, date.getFullYear(), date.getMonth(), date.getDate()) == true) ||
           (cal.getDateStatus(date, date.getFullYear(), date.getMonth(), date.getDate()) == "disabled"))) {
          return false;
        }
        break;
    }
...
};Zapatec.Calendar._keyEvent=function(ev){if(!window.calendar){return false;}
